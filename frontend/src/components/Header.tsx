            ...
            <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
                <div className="relative flex h-16 items-center justify-between">
                    <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
                        {/* Mobile menu button */}
                        <DisclosureButton className="group relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-white/5 hover:text-white focus:outline-2 focus:-outline-offset-1 focus:outline-indigo-500">                            <span className="absolute -inset-0.5" />
                            ...
                            <div className="flex space-x-8 items-center">
                                ...
                                {navigation.filter(item => item.name !== 'Home').map((item) => (
                                    <a 
                                        ...
                                        href={item.href}
                                        className={classNames(
                                            currentPath === item.href ? 'bg-green-400 text-white' : 'text-green-300 hover:bg-white/5 hover:text-white',
                                            'rounded-md px-3 py-2 text-lg font-medium',
                                        )}
                                    >
                                        {item.name}
          ...
          <div className="border-t border-emerald-700 pt-2 mt-2">
            {navigation.filter(item => item.name !== 'Home').map((item) => (
              <DisclosureButton
                ...
                className={classNames(
                  currentPath === item.href ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-white/5 hover:text-white',
                  'block rounded-md px-3 py-2 text-base font-medium',
                )}
              >
                {item.name}
